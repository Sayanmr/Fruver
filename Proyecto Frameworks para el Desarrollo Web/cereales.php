...
$sql = "SELECT id, nombre, precio FROM productos WHERE tipo = 'Cereal'";
...
<body>
    ...
    <div class="titulo cereales">
        <h1 class="titulo-h1">Cereales</h1>
    </div>
    <section id="products-list" class="products-list">
        <?php
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo '<div class="product-card c">';
                echo '<img src="./img/cereales/' . strtolower($row['nombre']) . '.png" alt="' . $row['nombre'] . '" class="product-img">';
