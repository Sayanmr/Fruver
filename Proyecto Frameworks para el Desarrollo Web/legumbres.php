...
$sql = "SELECT id, nombre, precio FROM productos WHERE tipo = 'Legumbre'";
...
<body>
    ...
    <div class="titulo legumbres">
        <h1 class="titulo-h1">Legumbres</h1>
    </div>
    <section id="products-list" class="products-list">
        <?php
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo '<div class="product-card l">';
                echo '<img src="./img/legumbres/' . strtolower($row['nombre']) . '.png" alt="' . $row['nombre'] . '" class="product-img">';
