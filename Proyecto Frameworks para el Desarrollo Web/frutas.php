...
$sql = "SELECT id, nombre, precio FROM productos WHERE tipo = 'Fruta'";
...
<body>
    ...
    <div class="titulo frutas">
        <h1 class="titulo-h1">Frutas</h1>
    </div>
    <section id="products-list" class="products-list">
        <?php
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo '<div class="product-card f">';
                echo '<img src="./img/frutas/' . strtolower($row['nombre']) . '.png" alt="' . $row['nombre'] . '" class="product-img">';
